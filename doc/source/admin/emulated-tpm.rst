...
of guests with emulated vTPM for the ``host`` and ``deployment`` TPM secret
security modes.
...
* Rebuild, evacuate, shelve, and rescue of servers with vTPMs is not currently
  supported. Live migration with vTPMs is only supported for the ``host`` and
  ``deployment`` TPM secret security modes.
