...
class LibvirtLiveMigrateData(LiveMigrateData):
    ...
    def obj_make_compatible(self, primitive, target_version):
        ...
        if (target_version < (1, 14)):
            ...
            # it is set to anything (including None, if the field is nullable).
            ...
            if any(field in vtpm_fields for field in primitive):
                ...
                # We raise here instead of silently removing the fields during
                # a backport because an older compute will not be able to honor
                # the TPM secret security policy required by the request.
                raise exception.ObjectActionError(
                    action='obj_make_compatible',
                    reason='%s are not supported in version %s' %
                       (' and '.join(vtpm_fields), target_version))
