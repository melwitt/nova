...
class ComputeManager(manager.Manager):
    ...
    def _set_tpm_secret_security(
        ...
        :param confirmed: Whether the TPM secret security policy has been
            ...
            this should be False and the security policy will be considered
            provisional until the user confirms it with a hard reboot.
...
>>>>>>> 8e88c33e51 (TPM: migrate legacy instances to new security policy)
        try:
            # NOTE(gibi): If ironic and vcenter virt driver slow start time
            # becomes problematic here then we should consider adding a config
            # option or a driver flag to tell us if we should thread
