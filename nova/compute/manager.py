...
class ComputeManager(manager.Manager):
    ...
    def _rebuild_default_impl(
        ...
        if evacuate:
            ...
            self.reset_ephemeral_encryption_bdms_sysmeta(
                instance, image_meta, bdms)
    ...
    @staticmethod
    def reset_ephemeral_encryption_bdms_sysmeta(
        instance: 'objects.Instance',
        image_meta: 'objects.ImageMeta',
        bdms: 'objects.BlockDeviceMappingList'
    ) -> None:
        """Reset ephemeral encryption attributes in instance system metadata
        ...
        pull them from the instance system metadata.

        The image properties in the system metadata need to be updated if the
        instance is being booted with a different image than the one with which
        it was first booted. An example is shelve/unshelve. When an instance is
        shelved, a snapshot is taken and after it is offloaded, the virt guest
        is destroyed, and resources are freed.
        ...
        prefix = utils.SM_IMAGE_PROP_PREFIX
        ...
        for key in (
            ...
            value = image_meta.properties.get(key)
            if value:
                sysmeta[prefix + key] = value
