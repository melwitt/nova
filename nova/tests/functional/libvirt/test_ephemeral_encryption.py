...
class EphemeralEncryptionTestRebuild(EphemeralEncryptionTestBase):
    ...
    def test_rebuild_server_encryption_from_flavor(self):
        compute = self.start_compute()
        driver = self.computes[compute].driver
        self._run_periodics()
        ...
        # We should still have three key manager secrets and three libvirt
        # secrets and they should be the same ones from earlier.
        keymgr_secrets_after_rebuild = self._get_key_mgr_secrets(ctx)
        self.assertEqual(keymgr_secrets, keymgr_secrets_after_rebuild)
        self.assertSecretsMatch(driver, bdms, keymgr_secrets_after_rebuild)
    ...
    def test_rebuild_server_encryption_from_image(self):
        compute = self.start_compute()
        driver = self.computes[compute].driver
        self._run_periodics()
    ...
    def test_rebuild_server_encryption_change_rejected(self):
        compute = self.start_compute()
        driver = self.computes[compute].driver
        self._run_periodics()
