...
class VTPMServersTest(base.LibvirtMigrationMixin, base.ServersTestBase):
    ...
    def _assert_legacy_server_migrated_secret_security(self, server, security):
        ctx = nova_context.get_admin_context()
        instance = objects.Instance.get_by_uuid(ctx, server['id'])
        self.assertEqual(
            security,
    ...
    def test_create_legacy_server_secret_security_host_deployment(
            self, secret_security, mock_svc_user_context):
        ...
        # A legacy secret should have ephemeral=yes and private=yes.
        ctx = nova_context.get_admin_context()
        instance = objects.Instance.get_by_uuid(ctx, server['id'])
        conn = compute.driver._host.get_connection()
        ...
        # not yet confirmed. If the 'host' policy had been acted upon, we would
        # find a libvirt secret with ephemeral=no and private=no.
        self._assert_libvirt_had_secret(compute, secret_uuid)
    ...
    def test_live_migrate_server_different_secret_security_default(self):
        """Live migration when hosts have different supported secret security.

        Test a scenario where a server is created with vTPM but without a
        specific secret security mode. The server will be assigned the host
        default. Then try to live migrate it to a host which does not support
        the secret security of the instance. This should fail to schedule as
        there is no valid host available that supports the needed secret
        security mode.
        """
        # Source with 'host' TPM secret security.
        # This will make the source report trait:
        # COMPUTE_SECURITY_TPM_SECRET_SECURITY_HOST.
        self.flags(supported_tpm_secret_security=['host'], group='libvirt')
        self.flags(default_tpm_secret_security='host', group='libvirt')
        ...
        # Destination with 'deployment' TPM secret security.
        # This will make the destination report trait:
        # COMPUTE_SECURITY_TPM_SECRET_SECURITY_DEPLOYMENT.
        self.flags(
           supported_tpm_secret_security=['deployment'], group='libvirt')
