...
class VTPMServersTest(base.LibvirtMigrationMixin, base.ServersTestBase):
    ...
    def test_tpm_secret_security_legacy_instance(self):
        ...
        ctx = nova_context.get_admin_context()
        instance = objects.Instance.get_by_uuid(ctx, server['id'])
        self.assertNotIn('image_tpm_secret_security', instance.system_metadata)
        ...
        self.assertEqual(
            'host',
            instance.system_metadata['image_tpm_secret_security'])
        self.assertEqual(
            'False',
    ...
    def test_live_migrate_server_different_secret_security_flavor(self):
        # Do the test and use flavor or image to request secret security. This
        ...
        # FIXME: need to finish implementing 'deployment' secret security
        # first.
        return
