    ...
    def _test_generate_resized_backing_files(self, mock_utime, mock_copy,
        ...
        mock_extend.assert_called_once_with(
            imgmodel.LocalFileImage(self.QCOW2_BASE,
                                    ...
                                    encryption=encryption)
        mock_exist.assert_has_calls(exist_calls)
        # encryption=None here because this was the fetch_func call and the
        # source image was not encrypted.
        fn.assert_called_once_with(
            target=self.TEMPLATE_PATH, encryption=None, context=self.CONTEXT)
        self.assertTrue(mock_sync.called)
        self.assertFalse(mock_create.called)
        mock_utime.assert_called()
