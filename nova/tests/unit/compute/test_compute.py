...
class ComputeTestCase(BaseTestCase,
    ...
    def test_complete_deletion(self):
        ...
        with (
            mock.patch.object(
                self.compute, '_update_resource_tracker') as mock_update_rt,
            mock.patch.object(
                self.compute.reportclient, 'delete_allocation_for_instance')
                as mock_delete_alloc,
            mock.patch.object(
                self.compute, '_clean_instance_console_tokens')
                as mock_clean_tokens,
            mock.patch.object(
                self.compute, '_delete_scheduler_instance_info')
                as mock_delete_scheduler_info,
            mock.patch(
                'nova.crypto.delete_encryption_secret') as mock_delete_secret
        ):
            ...
            call = mock.call(ctxt, instance, uuids.secret)
