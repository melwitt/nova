...
class LibvirtDriver(driver.ComputeDriver):
    ...
    def _cleanup_ephemeral_encryption_secrets(
        ...
        # Cross cell resize passes block_device_info=None intentionally to
        # driver.cleanup() in order to prevent the driver from trying to unplug
        # VIFs or disconnect volumes after they have already been unplugged or
        # disconnected earlier when the guest was destroyed in
        # prep_snapshot_based_resize_at_source(). See
        # _confirm_snapshot_based_resize_at_source() for more details.
    ...
    def _cleanup_resize(self, context, migration, instance, network_info):
        ...
        if (migration is not None and
            ...
            # Destroy libvirt secrets for ephemeral encryption.
                ...
                try:
                    ...
                    if live_snapshot:
                        ...
                        root_disk.snapshot_extract(
                            out_path, image_format, src_encryption=encryption,
    ...
    def _live_snapshot(self, context, instance, guest, disk_path, out_path,
        ...
        if encryption and source_format == 'raw':
            source_format = 'luks'
        ...
        try:
            ...
            if not encryption:
                ...
                # NOTE(melwitt): blockRebase cannot work with encrypted disks
                # because there is no way to provide disk secrets to it.
                # blockCopy, which is a superset of blockRebase, can be used
                # instead.
            ...
            if secret_uuid:
                self._host.delete_secret('volume', secret_uuid)

        # Convert the delta (CoW) image with a backing file to a flat
        # image with no backing file.
        if encryption and image_format == 'raw':
            image_format = 'luks'
        libvirt_utils.extract_snapshot(
            disk_delta, 'qcow2', out_path, image_format,
            src_encryption=encryption, dest_encryption=encryption)
    ...
    def _hard_reboot(self, context, instance, network_info,
        ...
        if context.auth_token is not None:
            ...
            self._create_images_and_backing(
                context, instance, instance_dir, backing_disk_info,
                block_device_info=block_device_info)
    ...
    def _add_ephemeral_encryption_driver_bdm_attrs(
        ...
        if ('image_id' in driver_bdm and CONF.libvirt.images_type in
            ...
            # NOTE(melwitt): If our version of Ceph doesn't support a child
            # image having a different encryption key from its parent image, we
            # will need to use a copy of the source image secret
            # (backing_encryption_secret_uuid) for the child image.
            if (backing_secret_uuid and CONF.libvirt.images_type == 'rbd' and
    ...
    def _rebase_original_qcow2_image(self, context, instance, backend):
        ...
        if image_meta is not None:
            ...
            # the rebase if so.
            image_secret_uuid = image_meta.properties.get('os_encrypt_key_id')
            if image_secret_uuid and encryption:
    ...
    def _get_ephemeral_encryption_from_image(
            ...
            image_id: str,
            instance: 'objects.Instance',
    ) -> ty.Tuple['objects.ImageMeta', ty.Optional[EncryptionInfo]]:
        ...
        return image_meta, image_encryption

    def _try_fetch_image_cache(self, image, fetch_func, context, filename,
                               image_id, instance, size,
                               fallback_from_host=None):
        image_meta, image_encryption = (
            self._get_ephemeral_encryption_from_image(
    ...
    def _create_images_and_backing(self, context, instance, instance_dir,
        ...
        for info in disk_info:
            ...
            if (block_device_info is not None and
                ...
                # disk_info_mapping key naming format
                # (it uses '<uuid>_disk' instead of 'disk').
    ...
    def _get_instance_disk_info_from_config(self, guest_config,
        ...
        for device in guest_config.devices:
            ...
            if disk_type == 'file' and driver_type == 'ploop':
                ...
                if device.ephemeral_encryption is None:
                    dk_size = os.stat(path).st_blocks * 512
                    virt_size = os.path.getsize(path)
                else:
                    # NOTE(melwitt): Using os.stat and os.path.getsize to
                    # determine encrypted raw image size results in a size that
                    # does not match the size expected by libvirt during live
                    # migration. If the source and destination image sizes
                    # aren't the same from libvirt's perspective, the following
                    # error is raised from virDomainMigrateToURI3:
                    #   libvirtError: operation failed: migration of disk vda
                    #     failed: Source and target image have different sizes
                    qemu_img_info = disk_api.get_disk_info(path)
                    dk_size = qemu_img_info.disk_size
                    virt_size = qemu_img_info.virtual_size
                backing_file = ""
    ...
    def confirm_migration(self, context, migration, instance, network_info):
        ...
        self._cleanup_resize(context, migration, instance, network_info)
