...
class Image(metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        if (CONF.ephemeral_storage_encryption.enabled and
                # TODO(melwitt): Is this a bad thing to do?
                not isinstance(self, Lvm)):
            ...
            if not CONF.workarounds.disable_deep_image_inspection:
                ...
                if not matching_targets:
                    ...
                    LOG.debug(
                        ...
                        f'fingerprint "{filename}" in the cache. Proceeding '
                        f'to download image {image_id} from the image '
                        'service.')
                    ...
                    if disk_format in format_inspector.ALL_FORMATS:
                        ...
                        # This performs image inspection only. It does not
    ...
    def create_root(
        ...
        try:
            ...
            # TODO(melwitt): If the image is no longer in Glance, we won't know
            # what file extension to append to the target to copy from another
            # compute host ... i.e. what if the cached image on the other
            # compute host is <target>.gpt? We won't find it.
            target = self._get_or_create_base_image_path(filename)
    ...
    def get_disk_size(self, name):
        return disk_api.get_disk_size(name)
...
class Lvm(Image):
    ...
    def create_image(self, ctxt, base, size, image_id=None):
        ...
        def create_lvm_image(base, size):
            base_size = disk_api.get_disk_size(base)
...
class Rbd(Image):
    ...
    def create_root(
        ...
        @utils.synchronized(filename, external=True, lock_path=self.lock_path)
        def clone_fallback_to_fetch_sync():
            try:
